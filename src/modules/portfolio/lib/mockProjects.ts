...
/**
 * Mock project data - your actual live projects
 * No database required! Just update this file to add/edit projects.
 */
...
/**
 * Helper functions to work with mock data (simulating database queries)
 */
...
export async function getFeaturedMockProjects() {
  return Promise.resolve(mockProjects.filter((p) => p.featured));
}

export async function getMockProjectBySlug(slug: string) {
  return Promise.resolve(mockProjects.find((p) => p.slug === slug) || null);
}

export async function getMockProjectsByCategory(category: string) {
  return Promise.resolve(mockProjects.filter((p) => p.category === category));
}
